...
class PostSection extends Component {
        ...
        if (this.props.isIndividual == false) {
            ...
            .then(posts => {
                    postList  = posts.data.map(post => {
                        return (
                            <Post title={post.title} content={post.blogContent} key={post._id} link={post.link} author={post.author} datePosted={post.datePosted} />
                        )
                    });
                this.setState({
            ...
            axios.post('/post/single', { "title" : this.props.match.params.title})
                .then(post => {
                    postList  = (<Post title={post.data.title} content={post.data.blogContent} key={post.data._id} link={post.data.link} author={post.data.author} datePosted={post.data.datePosted} />)
                    this.setState({
                        posts : [post.data]
                    });
    ...
    render() {
        ...
        return (
            <>
                <WrapperBg className="container-fluid p-0" id="postWrapper" style={{backgroundImage : "url(" + WrapperImg + ")"}}>
                    <WrapperWhiteOverlay className="container-fluid m-0 p-0">
                        <div className="row">
                            <div className="col-12 col-md-9 pr-0">
                                { postList }
                            </div>
                            <div className="col-3 d-none d-md-flex d-lg-flex d-xl-flex pl-0">
